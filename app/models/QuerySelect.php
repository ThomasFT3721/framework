...
class QuerySelect implements QueryInterface
{
    ...
    {
        if ($database === null) {
            $database = EnvironmentVariable::get(EnvironmentVariablesIdentifiers::DB_DATABASE);
        }
    ...
    public function where(mixed ...$parameters): self
    {
        if (count($parameters) == 0) {
            throw new \InvalidArgumentException("Invalid number arguments");
        }
    ...
    public function addParameter(mixed $key, mixed $value): self
    {
        if ($this->params === null) {
            $this->params = [];
        }
